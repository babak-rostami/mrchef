<div>
    ...
    <div id="bf-image-picker"
        class="relative w-full h-48 mt-2 border-2 border-dashed border-gray-300 rounded-lg
               flex items-center justify-center cursor-pointer overflow-hidden
               hover:bg-gray-50 transition">

        <!-- عکس -->
        <img id="bf-main-image-show" class="{{ isset($bimage_src) ? '' : 'hidden' }} h-full"
            src="{{ $bimage_src ?? '' }}">

        <!-- placeholder -->
        <div id="bf-image-placeholder"
            class="flex flex-col items-center text-gray-500
                    {{ isset($bimage_src) ? 'hidden' : '' }}">
        ...
        <div id="bf-image-overlay"
            class="absolute inset-0 bg-black/20 text-white text-2xl font-bold 
                    flex items-center justify-center {{ isset($bimage_src) ? '' : 'hidden' }}
